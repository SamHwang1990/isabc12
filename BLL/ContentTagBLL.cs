 ...
 * 类 名 称：ContentTag (ContentTagBLL)
 * 功能描述：
 * 
******************************************************************/
...
namespace Hope.BLL
{
	...
	/// <summary>
    /// Business Handler ContentTag
    /// </summary>
    public class ContentTagBLL : BaseBLL
    {
		/// <summary>
        /// 数据访问层接口 IQuery
        /// </summary>
        ...
        private ContentTagDAL Provider;
		
		private static ContentTagBLL instance = null;
		
		/// <summary>
        /// Business Handler ContentTagHandler 
        /// </summary>
        private ContentTagBLL()
        {
			query = new QueryStringBuilder<ContentTagData, int>("Content_Tag", "TagID");
            Provider = new ContentTagDAL(ApplicationConfig.DBConnectionString);
        ...
        /// 获取实例
        /// </summary>
		public static ContentTagBLL GetInstance()
		{
			if(instance == null)
			{
				instance = new ContentTagBLL();
			}
        ...
        /// </summary>
        /// <returns>return the handler result</returns>
        public bool Remove(int TagID)
        {
            HandlerMessage.Code = "00";
            HandlerMessage.Text = "删除成功！";
			HandlerMessage.Succeed = true;
            if (!Provider.Delete(TagID))
            {
                HandlerMessage.Code = "01";
        ...
        /// <returns>return the handler result</returns>
        public bool Remove(List<int> TagIDList)
        {
            HandlerMessage.Code = "00";
            HandlerMessage.Text = "删除成功！";
			HandlerMessage.Succeed = true;
            if (!Provider.Delete(TagIDList))
            {
                HandlerMessage.Code = "01";
                HandlerMessage.Text = "删除失败！";
				HandlerMessage.Succeed = false;
        ...
        {
            return Provider.GetDataById(TagID);
        }
        ...
        /// <returns></returns>
        public ContentTagData GetDataByName(string tagName)
        {
            query.Clear();
            SimpleExpression exp = new SimpleExpression("TagName", tagName, "=");
            query.AddExp(exp);
        ...
        /// </summary>
        /// <returns></returns>
        public List<ContentTagData> GetDatas()
        {
        ...
        /// <returns></returns>
        public List<ContentTagData> GetList(int startIndex, int pageSize)
        {
        ...
        /// <returns></returns>
        public List<ContentTagData> GetList(int startIndex, int pageSize,string orderColumn, ColumnOrderType orderType)
        {
			int endIndex = startIndex + pageSize - 1; //当前页要显示的记录的结束索引
