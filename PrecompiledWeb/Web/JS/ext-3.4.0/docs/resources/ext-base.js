...
(function () {
    ...
    Ext.apply(Ext, {SSL_SECURE_URL:c && r ? 'javascript:""' : "about:blank", isStrict:l, isSecure:c, isReady:false, enableGarbageCollector:true, enableListenerCollection:false, enableNestedListenerRemoval:false, USE_NATIVE_JSON:false, applyIf:function (C, D) {
        ...
        var D = [];
        D.push("pkgs/ext-all-no-core.js");
        Ext.Loader.load(D, E, C)
    }, getDom:function (D, C) {
        if (!D || !i) {
            return null
        }
...
(function () {
    ...
    Ext.lib.Ajax = function () {
        ...
        }, serializeForm:function (v) {
            var u = v.elements || (document.forms[v] || Ext.getDom(v)).elements, q = false, t = encodeURIComponent, r, w = "", s;
            Ext.each(u, function (x) {
                r = x.name;
                s = x.type;
                if (!x.disabled && r) {
                    if (/select-(one|multiple)/i.test(s)) {
                        Ext.each(x.options, function (y) {
                            if (y.selected) {
                                w += String.format("{0}={1}&", t(r), t((y.hasAttribute ? y.hasAttribute("value") : y.getAttribute("value") !== null) ? y.value : y.text))
                            }
                        })
                    } else {
                        if (!(/file|undefined|reset|button/i.test(s))) {
                            if (!(/radio|checkbox/i.test(s) && !x.checked) && !(s == "submit" && q)) {
                                w += t(r) + "=" + t(x.value) + "&";
                                q = /submit/i.test(s)
                            }
                        }
