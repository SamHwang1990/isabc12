        ...
        b.onWrite = b.onWrite.createInterceptor(function (h, i, f, e) {
            ...
            switch (f.status) {
                case 200:
                    ...
                    break;
                case 201:
                    if (Ext.isEmpty(g.raw.responseText)) {
                        g.success = true
                    } else {
                        return true
                    }
            ...
            if (g.success === true) {
                this.fireEvent("write", this, h, g.data, g, e, i.request.arg)
            } else {
                this.fireEvent("exception", this, "remote", h, i, g, e)
            }
...
Ext.data.Store = Ext.extend(Ext.util.Observable, {writer:undefined, remoteSort:false, autoDestroy:false, pruneModifiedRecords:false, lastOptions:null, autoSave:true, batch:true, restful:false, paramNames:undefined, defaultParamNames:{start:"start", limit:"limit", sort:"sort", dir:"dir"}, isDestroyed:false, hasMultiSort:false, batchKey:"_ext_batch_", constructor:function (a) {
    ...
    var d = this.modified, g = b.length, a, f;
    for (f = 0; f < g; f++) {
        a = b[f];
    ...
    var c = this.batches, e = this.batchKey + d, h = c[e], a;
    if (h) {
        a = h.data[g] || [];
        h.data[g] = a.concat(f);
        if (h.count === 1) {
    ...
    return(c == "read") ? this.loadRecords : function (f, e, g) {
        this["on" + Ext.util.Format.capitalize(c) + "Records"](g, a, [].concat(f));
        if (g === true) {
    ...
    if (!b || g === false) {
        if (g !== false) {
            this.fireEvent("load", this, [], k)
        }
        if (k.callback) {
            k.callback.call(k.scope || this, [], k, false, b)
        }
        return
    }
    ...
    if (k.callback) {
        k.callback.call(k.scope || this, a, k, true)
    }
    ...
    var a = this.hasMultiSort ? this.multiSortInfo : this.sortInfo, h = a.direction || "ASC", g = a.sorters, c = [];
    if (!this.hasMultiSort) {
        g = [
            {direction:h, field:a.field}
        ]
    }
    for (var d = 0, b = g.length; d < b; d++) {
        c.push(this.createSortFunction(g[d].field, g[d].direction))
    }
    if (c.length == 0) {
        return
    }
    ...
    var e = function (m, l) {
        var k = c[0].call(this, m, l);
        if (c.length > 1) {
            for (var o = 1, n = c.length; o < n; o++) {
                k = k || c[o].call(this, m, l)
            }
        }
        return f * k
    };
...
}, singleSort:function (f, c) {
    var e = this.fields.get(f);
    if (!e) {
        return false
    }
    ...
    return function (b) {
        ...
        for (var d = 0, c = a.length; d < c; d++) {
            var g = a[d], f = g.fn, e = g.scope;
            h = h && f.call(e, b)
        }
        return h
    }
}, filter:function (m, l, g, h, e) {
    var k;
    if (Ext.isObject(m)) {
        m = [m]
    }
    if (Ext.isArray(m)) {
        ...
        for (var f = 0, d = m.length; f < d; f++) {
            var a = m[f], c = a.fn, n = a.scope || this;
            if (!Ext.isFunction(c)) {
        ...
        k = this.createFilterFn(m, l, g, h, e)
    }
    ...
    var g = (b === true && this.snapshot) ? this.snapshot.items : this.data.items;
    var k, m, a = [], c = {};
...
Ext.data.DataWriter.prototype = {writeAllFields:false, listful:false, apply:function (e, f, d, a) {
    var c = [], b = d + "Record";
    if (Ext.isArray(a)) {
    ...
    this.render(e, f, c)
}, render:Ext.emptyFn, updateRecord:Ext.emptyFn, createRecord:Ext.emptyFn, destroyRecord:Ext.emptyFn, toHash:function (f, c) {
    var e = f.fields.map, d = {}, b = (this.writeAllFields === false && f.phantom === false) ? f.getChanges() : f.data, a;
    ...
    if (f.phantom) {
        if (f.fields.containsKey(this.meta.idProperty) && Ext.isEmpty(f.data[this.meta.idProperty])) {
...
Ext.extend(Ext.data.DataProxy, Ext.util.Observable, {restful:false, setApi:function () {
    ...
    f = f || {};
    if ((e === Ext.data.Api.actions.read) ? this.fireEvent("beforeload", this, f) : this.fireEvent("beforewrite", this, e, b, f) !== false) {
        this.doRequest.apply(this, arguments)
    } else {
        g.call(d || this, null, c, false)
    }
}, load:null, doRequest:function (e, b, f, a, g, d, c) {
    this.load(f, a, g, d, c)
...
Ext.extend(Ext.data.ScriptTagProxy, Ext.data.DataProxy, {timeout:30000, callbackParam:"callback", nocache:true, doRequest:function (e, f, d, g, i, j, k) {
...
Ext.extend(Ext.data.HttpProxy, Ext.data.DataProxy, {getConnection:function () {
    ...
    var g = {method:(this.api[f]) ? this.api[f]["method"] : undefined, request:{callback:b, scope:e, arg:a}, reader:c, callback:this.createCallback(f, d), scope:this};
    if (h.jsonData) {
    ...
    if (d.success === true) {
        this.fireEvent("write", this, f, d.data, d, b, h.request.arg)
    } else {
...
Ext.extend(Ext.data.MemoryProxy, Ext.data.DataProxy, {doRequest:function (b, c, a, d, g, h, i) {
    a = a || {};
    var j;
    try {
