...
Ext.extend(Ext.data.DirectProxy, Ext.data.DataProxy, {paramOrder:undefined, paramsAsHash:true, directFn:undefined, doRequest:function (b, c, a, e, j, k, m) {
    var h = [], g = this.api[b] || this.directFn;
    switch (b) {
    ...
    var l = {params:a || {}, request:{callback:j, scope:k, arg:m}, reader:e};
...
Ext.direct.JsonProvider = Ext.extend(Ext.direct.Provider, {parseResponse:function (a) {
    ...
    try {
        ...
        var d = new Ext.Direct.ExceptionEvent({data:g, xhr:h, code:Ext.Direct.exceptions.PARSE, message:"Error parsing json response: \n\n " + f});
        return[d]
    }
    var c = [];
    if (Ext.isArray(f)) {
...
Ext.direct.PollingProvider = Ext.extend(Ext.direct.JsonProvider, {priority:3, interval:3000, constructor:function (a) {
    ...
    if (h) {
        ...
        var f = new Ext.Direct.ExceptionEvent({data:f, code:Ext.Direct.exceptions.TRANSPORT, message:"Unable to connect to the server.", xhr:g});
        this.fireEvent("data", this, f)
    }
}});
...
Ext.direct.RemotingProvider = Ext.extend(Ext.direct.JsonProvider, {enableBuffer:10, maxRetries:1, timeout:undefined, constructor:function (a) {
    ...
    var g = this.actions;
    for (var h in g) {
        var d = this.namespace[h] || (this.namespace[h] = {}), e = g[h];
    ...
    if (g) {
        ...
        for (var b = 0, c = j.length; b < c; b++) {
            var d = j[b], k = this.getTransaction(d);
        ...
        for (var b = 0, c = f.length; b < c; b++) {
            ...
            if (k && k.retryCount < this.maxRetries) {
                ...
                var d = new Ext.Direct.ExceptionEvent({data:d, transaction:k, code:Ext.Direct.exceptions.TRANSPORT, message:"Unable to connect to the server.", xhr:h});
                this.fireEvent("data", this, d);
                if (k) {
    ...
    var f = {url:this.url, callback:this.onData, scope:this, ts:d, timeout:this.timeout}, b;
    if (Ext.isArray(d)) {
        b = [];
    ...
    var g = null, e = b[a.len], f = b[a.len + 1];
    if (a.len !== 0) {
        g = b.slice(0, a.len)
    }
    var d = new Ext.Direct.Transaction({provider:this, args:b, action:h, method:a.name, data:g, cb:f && Ext.isFunction(e) ? e.createDelegate(f) : e});
    ...
    var d = new Ext.Direct.Transaction({provider:this, action:i, method:b.name, args:[f, h, e], cb:e && Ext.isFunction(h) ? h.createDelegate(e) : h, isForm:true});
    if (this.fireEvent("beforecall", this, d, b) !== false) {
        Ext.Direct.addTransaction(d);
        var a = String(f.getAttribute("enctype")).toLowerCase() == "multipart/form-data", g = {extTID:d.tid, extAction:i, extMethod:b.name, extType:"rpc", extUpload:String(a)};
        Ext.apply(d, {form:Ext.getDom(f), isUpload:a, params:h && Ext.isObject(h.params) ? Ext.apply(g, h.params) : g});
    ...
    if (c && c.cb) {
        var b = c.cb, a = Ext.isDefined(f.result) ? f.result : f.data;
        if (Ext.isFunction(b)) {
            b(a, f)
        } else {
