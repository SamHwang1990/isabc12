...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {groupByText:"Group By This Field", showGroupsText:"Show in Groups", hideGroupedColumn:false, showGroupName:true, startCollapsed:false, enableGrouping:true, enableGroupingMenu:true, enableNoGroups:true, emptyGroupText:"(None)", ignoreAdd:false, groupTextTpl:"{text}", groupMode:"value", cancelEditOnToggle:true, initTemplates:function () {
    ...
    if (g) {
        var f = this.getGroupField(), d = this.getPrefix(f), a = g.id.substring(d.length), c = new RegExp("gp-" + Ext.escapeRe(f) + "--hd");
    ...
    var c = this.cm.config[b], d = h ? h.call(c.scope, a, {}, e, i, b, f) : String(a);
    if (d === "" || d === "&#160;") {
    ...
    var y = this.getGroupField(), n = this.cm.findColumnIndex(y), v, h = "width:" + this.getTotalWidth() + ";", e = this.cm.config[n], b = e.groupRenderer || e.renderer, s = this.showGroupName ? (e.groupName || e.header) + ": " : "", x = [], k, t, u, m;
    for (t = 0, u = f.length; t < u; t++) {
        var j = a + t, l = f[t], d = l.data[y];
        ...
        if (!k || k.group != v) {
            ...
            k = {group:v, gvalue:d, text:s + v, groupId:m, startRow:j, rs:[l], cls:this.state[m] ? "" : "x-grid-group-collapsed", style:h};
            x.push(k)
        } else {
            k.rs.push(l)
        }
    ...
    var b = this.cm.config[a], d = b.groupRenderer || b.renderer, f = (this.groupMode == "value") ? c : this.getGroup(c, {data:{}}, d, 0, a, this.ds);
    ...
    var h = [], c = this.getGroups(), f, e = 0, a = c.length, d, b;
    for (; e < a; ++e) {
        f = c[e].childNodes[1];
        if (f) {
            f = f.childNodes;
