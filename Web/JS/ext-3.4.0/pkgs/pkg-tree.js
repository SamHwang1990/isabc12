...
Ext.tree.TreeNodeUI = Ext.extend(Object, {constructor:function (a) {
    ...
    if (this.rendered) {
        var f = this.node, d, c, a = f.isLast() ? "x-tree-elbow-end" : "x-tree-elbow", e = f.hasChildNodes();
...
Ext.extend(Ext.tree.TreeLoader, Ext.util.Observable, {uiProviders:{}, clearOnLoad:true, paramOrder:undefined, paramsAsHash:false, nodeParameter:"node", directFn:undefined, load:function (b, c, a) {
    ...
    var e = Ext.apply({}, this.baseParams), g = this.nodeParameter, b = this.paramOrder;
    g && (e[g] = f.id);
    if (this.directFn) {
        var c = [f.id];
        if (b) {
...
Ext.tree.TreeSorter = Ext.extend(Object, {constructor:function (a, c) {
    ...
    var e = this.dir && this.dir.toLowerCase() == "desc", h = this.property || "text", d = this.sortType, g = this.folderSort, b = this.caseSensitive === true, f = this.leafAttr || "leaf";
    if (Ext.isString(d)) {
        d = Ext.data.SortTypes[d]
    }
    this.sortFn = function (n, l) {
        var j = n.attributes, i = l.attributes;
        if (g) {
            if (j[f] && !i[f]) {
                return 1
            }
        ...
        var m = j[h], k = i[h], p = d ? d(m) : (b ? m : m.toUpperCase()), o = d ? d(k) : (b ? k : k.toUpperCase());
        if (p < o) {
            return e ? 1 : -1
        } else {
            if (p > o) {
...
if (Ext.dd.DropZone) {
    ...
    Ext.extend(Ext.tree.TreeDropZone, Ext.dd.DropZone, {ddGroup:"TreeDD", expandDelay:1000, expandNode:function (a) {
        ...
        if (this.allowContainerDrop && this.isValidDropPoint({ddel:this.tree.getRootNode().ui.elNode, node:this.tree.getRootNode()}, "append", a, f, c)) {
        ...
        var f = {tree:this.tree, target:i, data:g, point:b, source:a, rawEvent:h, dropNode:d, cancel:!d, dropStatus:false};
